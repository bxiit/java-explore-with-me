...
public interface StatisticsRepository extends JpaRepository<Endpoint, Long> {
    @Query("""
            select new ru.practicum.explorewithme.statistics.contract.dto.ViewStats(eh.app, eh.uri, count(eh.ip)) from Endpoint eh
    ...
    @Query("""
            select new ru.practicum.explorewithme.statistics.contract.dto.ViewStats(eh.app, eh.uri, count(distinct eh.ip)) from Endpoint eh
            where eh.timestamp between ?1 and ?2
            group by eh.app, eh.uri
